...
function SetDirection(event, dir) {
...
document.addEventListener("DOMContentLoaded", function () {
  // Get all SCar elements
  const sCarElements = document.querySelectorAll(".ServiceBTN");

  // Add click event listener to each SCar element
  sCarElements.forEach(function (sCar) {
    sCar.addEventListener("click", function () {
      // Remove active class from all SCar elements
      sCarElements.forEach(function (element) {
        element.classList.remove("active");
      });

      // Add active class to the clicked SCar element
...
function selectOption(option) {
  ...
  const options = document.querySelectorAll('.segmented-control-option');
  ...
  if (option === 'individual') {
      control.classList.remove('corporate');
      options[0].classList.add('selected');
  } else {
      control.classList.add('corporate');
